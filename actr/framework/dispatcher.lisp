            ...
            (cond ((eq char :done)
                  ...
                  ((eq (char-code char) 4)
                   (let ((message (handler-case (json:decode-json-from-string s)
                      ...
                      ;add 2023/10/17 by nishikawa for debug
                      (format t "~A~%" message)
...
(eval-when (:load-toplevel :execute)
  (cond (*dispatcher* (error "The *dispatcher* variable is already set, but should be nil."))
        ...
        ;;受け取った*given-port*で起動する．細川くんのthe-game参照
        ((start-des t nil *given-port*) (format t "~%The ACT-R remote command dispatcher has been started~%"))
